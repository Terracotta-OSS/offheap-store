...
import com.terracottatech.offheapstore.util.PhysicalMemory;
...
public class UpfrontAllocatingPageSource implements PageSource {
    ...
    private static Collection<ByteBuffer> allocateBackingBuffers(BufferSource source, long max, int maxChunk, int minChunk, boolean fixed) {
      ...
      LOGGER.info("Took {} ms to create CacheManager off-heap storage of {}B.", TimeUnit.NANOSECONDS.toMillis(duration), DebuggingUtils.toBase2SuffixedString(max));
