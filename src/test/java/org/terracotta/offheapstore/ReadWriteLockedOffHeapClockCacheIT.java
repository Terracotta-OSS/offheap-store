...
public class ReadWriteLockedOffHeapClockCacheIT extends AbstractConcurrentOffHeapMapIT {
  ...
  public void testCacheEviction() {
    ...
    CacheTestRoutines.testCacheEviction(new ReadWriteLockedOffHeapClockCache<>(new UnlimitedPageSource(new OffHeapBufferSource()), factory
  ...
  public void testCacheEvictionDueToTableResizeFailure() {
    assumeThat(generator, is(GOOD_GENERATOR));
    for (int i = 1; i < 100; i++) {
      CacheTestRoutines.testCacheEvictionMinimal(new ReadWriteLockedOffHeapClockCache<>(new PhantomReferenceLimitedPageSource(16 * i), new SplitStorageEngine<>(new IntegerStorageEngine(), new IntegerStorageEngine()), 1));
    }
  }
  ...
  protected ConcurrentMap<SpecialInteger, SpecialInteger> createMap(Generator generator) {
    return new ReadWriteLockedOffHeapClockCache<>(new UnlimitedPageSource(new OffHeapBufferSource()), generator.engine(), 1);
  }

  @Override
  protected ConcurrentMap<Integer, byte[]> createOffHeapBufferMap(PageSource source) {
    assumeThat(generator, is(GOOD_GENERATOR));
    return new ReadWriteLockedOffHeapClockCache<>(source, new SplitStorageEngine<>(new IntegerStorageEngine(), new OffHeapBufferHalfStorageEngine<>(source, 1024, ByteArrayPortability.INSTANCE)));
  }
}
